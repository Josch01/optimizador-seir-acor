# -*- coding: utf-8 -*-
"""
SEIR + ACOR GUI (v5.2 - Corrección Final de Acento de Color)
---------------------------------------------------
Versión que reemplaza manualmente el color de acento para máxima
compatibilidad con versiones antiguas de qdarktheme.
"""
...
from clases.mcmc_dialog import MCMCDialog
from clases.model_config_dialog import ModelConfigDialog

# --- CLASE PARA EL DIÁLOGO DE SELECCIÓN DE TEMA MEJORADO ---
class ThemeSettingsDialog(QDialog):
    """
    Un diálogo para seleccionar el tema y el color de acento de la aplicación.
    """
    PREDEFINED_ACCENTS = [
        "#7750f8", "#007bff", "#28a745", "#dc3545",
        "#ffc107", "#17a2b8", "#6f42c1", "#f8f9fa"
    ]

    def __init__(self, current_theme="auto", current_accent="#7750f8", parent=None):
        super().__init__(parent)
        self.setWindowTitle("Configuración de Apariencia")
        self.selected_accent_color = current_accent
        self.accent_buttons = {}

        layout = QVBoxLayout(self)

        # --- Grupo de Tema ---
        theme_group_box = QGroupBox("Tema")
        theme_layout = QVBoxLayout(theme_group_box)
        self.radio_auto = QRadioButton("Automático (igual al sistema)")
        ...
        if current_theme == "light": self.radio_light.setChecked(True)
        elif current_theme == "dark": self.radio_dark.setChecked(True)
        else: self.radio_auto.setChecked(True)
        theme_group_box.setLayout(theme_layout)
        layout.addWidget(theme_group_box)

        # --- Grupo de Color de Acento ---
        accent_group_box = QGroupBox("Color de Acento")
        accent_layout = QHBoxLayout()
        accent_group_box.setLayout(accent_layout)
        accent_layout.setSpacing(10)
        
        for color_hex in self.PREDEFINED_ACCENTS:
            btn = QPushButton()
            btn.setFixedSize(24, 24)
            btn.setToolTip(color_hex.upper())
            btn.clicked.connect(partial(self._on_accent_selected, color_hex))
            accent_layout.addWidget(btn)
            self.accent_buttons[color_hex] = btn
        
        accent_layout.addStretch()
        layout.addWidget(accent_group_box)

        self._update_selected_accent_visuals()
    ...
    def _on_accent_selected(self, color_hex):
        self.selected_accent_color = color_hex
        self._update_selected_accent_visuals()

    def _update_selected_accent_visuals(self):
        for color, btn in self.accent_buttons.items():
            border_style = "3px solid dodgerblue" if color == self.selected_accent_color else "2px solid gray"
            btn.setStyleSheet(f"""
                QPushButton {{
    ...
    def get_settings(self):
        """Devuelve el tema y el color de acento seleccionados."""
        theme = "auto"
        if self.radio_light.isChecked(): theme = "light"
        elif self.radio_dark.isChecked(): theme = "dark"
        return theme, self.selected_accent_color

# --- FIN DE LA CLASE DE DIÁLOGO ---
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.current_theme_name = "auto"
        self.current_accent_color = self.DEFAULT_ACCENT
        self.is_dark_resolved = False
    ...
    def init_ui(self):
        self.setWindowTitle("Interfaz de Optimización SEIR (v5.2)")
    ...
    def _setup_config_tab(self, tab):
        ...
        btn_config_model = QPushButton("Configurar Estructura..."); btn_config_model.clicked.connect(self.open_model_config_dialog); p_layout.addWidget(btn_config_model)
        btn_params = QPushButton("Ajustar Límites de Parámetros"); btn_params.clicked.connect(self.open_params_dialog); p_layout.addWidget(btn_params)
    ...
    def _setup_dashboard_tab(self, tab):
        ...
        self.history_table.setHorizontalHeaderLabels(["ID", "Costo Final", "AIC", "BIC", "Iteraciones", "Tiempo (s)"])
        ...
        btn_mcmc = QPushButton("Análisis MCMC"); btn_mcmc.clicked.connect(self.show_mcmc_dialog)
        btn_mcmc.setToolTip("Ejecutar un análisis MCMC para explorar la incertidumbre de los parámetros (avanzado).")

        plots_layout.addWidget(btn_rt, 0, 0); plots_layout.addWidget(btn_residuals, 0, 1)
        plots_layout.addWidget(btn_convergence, 1, 0); plots_layout.addWidget(btn_distribution, 1, 1)
        plots_layout.addWidget(btn_parallel, 2, 0); plots_layout.addWidget(btn_bounds, 2, 1)
        plots_layout.addWidget(btn_mcmc, 3, 0, 1, 2)
        layout.addWidget(dashboard_plots_group, 1); layout.addStretch(1)

    def open_model_config_dialog(self):
        """Abre el diálogo para configurar la estructura del modelo (nº de armónicos)."""
        dlg = ModelConfigDialog(self.model.harmonic_config, self)
        if dlg.exec_():
            new_config = dlg.get_config()
            if new_config != self.model.harmonic_config:
                self.log("Reconfigurando la estructura del modelo...", "purple")
                self.model.configure_model(new_config)
                num_params = self.model.DIM
                self.log(f"Nueva estructura de modelo cargada: {num_params} parámetros.", "purple")
                QMessageBox.information(self, "Estructura del Modelo Cambiada",
                                      f"La estructura del modelo ha sido actualizada a {num_params} parámetros. "
                                      "Los resultados de ejecuciones anteriores ya no son comparables. "
                                      "Se recomienda limpiar la sesión (Archivo > Limpiar Sesión) antes de ejecutar una nueva optimización.")
    ...
    def _create_menu_bar(self):
        ...
        file_menu.addSeparator()
        file_menu.addAction("Limpiar Sesión", self._reset_session_state)
        file_menu.addSeparator()
        file_menu.addAction("&Salir", self.close)

        view_menu = menu_bar.addMenu("&Ver")
        theme_settings_action = QAction("Apariencia...", self, triggered=self.open_theme_dialog)
        view_menu.addAction(theme_settings_action)
    ...
    def _load_settings(self):
        ...
        self.current_theme_name = self.settings.value("theme", "auto", type=str)
        self.current_accent_color = self.settings.value("accent_color", self.DEFAULT_ACCENT, type=str)
        self._set_theme(self.current_theme_name, self.current_accent_color)

    def _save_settings(self):
        self.settings.setValue("geometry", self.saveGeometry())
        self.settings.setValue("windowState", self.saveState())
        self.settings.setValue("theme", self.current_theme_name)
        self.settings.setValue("accent_color", self.current_accent_color)
        self.settings.setValue("recent_files", self.recent_files)

    def _set_theme(self, theme_name="auto", accent_color=None):
        ...
        if theme_name == "auto":
            ...
            theme_to_load = "dark" if bg_color.lightness() < 128 else "light"

        # Carga la hoja de estilos y reemplaza el color primario por defecto
        stylesheet = qdarktheme.load_stylesheet(theme_to_load)
        modified_stylesheet = stylesheet.replace("#7750f8", self.current_accent_color)

        app = QApplication.instance()
        if app:
            app.setStyleSheet(modified_stylesheet)
            bg_color = app.palette().color(QPalette.Window)
            self.is_dark_resolved = bg_color.lightness() < 128
        else:
            self.is_dark_resolved = theme_to_load == "dark"
            
        pg.setConfigOption('background', '#2b2b2b' if self.is_dark_resolved else 'w')
        pg.setConfigOption('foreground', '#f0f0f0' if self.is_dark_resolved else 'k')
        self.update_plot()

    def open_theme_dialog(self):
        dlg = ThemeSettingsDialog(self.current_theme_name, self.current_accent_color, self)
        if dlg.exec_():
            new_theme, new_accent = dlg.get_settings()
            self._set_theme(new_theme, new_accent)

    def log(self, txt, color="black"):
        timestamp = time.strftime("%H:%M:%S")
        if self.is_dark_resolved and color == "black": 
            color = "#f0f0f0"
        
        # Usa el color de acento para los mensajes "blue"
        log_color = self.current_accent_color if color == "blue" else color
        
        self.log_text.append(f'<font color="{log_color}">[{timestamp}] {txt}</font>')
    ...
    def _show_about_dialog(self):
        QMessageBox.about(self, "Acerca de SEIR ACOR Optimizer",
                            "<p><b>SEIR ACOR Optimizer v5.2</b></p>"
    ...
    def _show_report_selection_dialog(self):
        ...
        if dialog.exec_() == QDialog.Accepted and list_widget.currentItem():
            ...
            self._add_dashboard_plot("rt", rt_plot_widget)
    ...
    def update_plot(self, comparison_params=None):
        ...
        if self.best_params_overall is not None and len(self.model.I_data) > 0:
            ...
            try:
                k = self.best_params_overall[-1]
                self.model.set_initial_conditions(k)
                sol = odeint(self.model.seir_harmonic, self.model.y0, t_sim, args=tuple(self.best_params_overall[:-1]), mxstep=200_000)
        ...
        if comparison_params is not None and len(self.model.I_data) > 0:
            t_sim = np.linspace(float(self.model.t_data[0]), float(self.model.t_data[-1]), 300)
            try:
                k = comparison_params[-1]
                self.model.set_initial_conditions(k)
                sol = odeint(self.model.seir_harmonic, self.model.y0, t_sim, args=tuple(comparison_params[:-1]), mxstep=200_000)
    ...
    def run_optimization(self):
        ...
        try:
            ...
            config_summary = f"<b>Iniciando Optimización #{self.run_counter + 1} ...</b>"
    ...
    def update_progress(self, progress, message, params):
        ...
        if "Best Cost:" in message:
            try: cost_str = message.split("Best Cost:")[1].split("(")[0].strip()
        ...
        if params is not None: self.update_plot(comparison_params=params)

    def optimization_finished(self, optimizer):
        self.run_counter += 1
        duration = time.time() - self.start_time
        result = RunResult(run_id=self.run_counter, best_cost=optimizer.best_cost_global, best_params=optimizer.best_params_global, cost_history=optimizer.history_best_cost, duration=duration)
        
        if result.best_cost != float('inf') and self.model.loss_type == "MSE":
            num_params = self.model.DIM
            num_data_points = len(self.model.I_data)
            aic, bic = self.model.calculate_aic_bic(result.best_cost, num_params, num_data_points)
            result.aic = aic
            result.bic = bic
        else:
            result.aic = float('nan')
        ...
        self.btn_run.setEnabled(True); self.btn_stop.setEnabled(False); self.export_pdf_action.setEnabled(True)
        if result.best_cost < self.best_cost_overall:
            self.best_cost_overall = result.best_cost; self.best_params_overall = result.best_params
            self.log("<b>¡Nuevo mejor resultado global encontrado!</b>", "green")
        self.worker = None; self.optimizer = optimizer
        self._update_dashboard(); self.update_plot()
        for dlg in self.dialog_windows:
            if hasattr(dlg, 'update_contents'): dlg.update_contents()

    def _update_dashboard(self):
        self.tabs.setTabEnabled(1, True); self.history_table.setRowCount(0)
        for result in reversed(self.run_history):
            row_pos = self.history_table.rowCount(); self.history_table.insertRow(row_pos)
            ...
            self.history_table.setItem(row_pos, 2, QTableWidgetItem(f"{result.aic:.2f}" if not np.isnan(result.aic) else "N/A"))
            self.history_table.setItem(row_pos, 3, QTableWidgetItem(f"{result.bic:.2f}" if not np.isnan(result.bic) else "N/A"))
            self.history_table.setItem(row_pos, 4, QTableWidgetItem(str(len(result.cost_history))))
            self.history_table.setItem(row_pos, 5, QTableWidgetItem(f"{result.duration:.2f}"))
        if self.history_table.rowCount() > 0:
            self.history_table.selectRow(0); self._display_comparison_run(0, 0)
    ...
    def _display_comparison_run(self, row, column):
        ...
        if result:
            ...
            self.update_plot(comparison_params=result.best_params); self._clear_dashboard_plots()
    ...
    def _plot_rt_on_widget(self, plot_widget, params):
        ...
        p = params[:-1] # Usar todos los parámetros excepto k
        p_idx = 0
        
        # Cálculo dinámico de Beta
        beta_terms = self.model.harmonic_config.get('beta', 0)
        log_beta_vals = p[p_idx]
        p_idx += 1
        for _ in range(beta_terms):
            log_beta_vals += p[p_idx] * np.cos(p[p_idx+1] * t_sim + p[p_idx+2])
            p_idx += 3
        beta_t = np.exp(log_beta_vals)

        # Cálculo dinámico de Gamma
        gamma_terms = self.model.harmonic_config.get('gamma', 0)
        log_gamma_vals = p[p_idx]
        p_idx += 1
        for _ in range(gamma_terms):
            log_gamma_vals += p[p_idx] * np.cos(p[p_idx+1] * t_sim + p[p_idx+2])
            p_idx += 3
        gamma_t = np.exp(log_gamma_vals)

        k = params[-1]
        self.model.set_initial_conditions(k)
        sol = odeint(self.model.seir_harmonic, self.model.y0, t_sim, args=tuple(p), mxstep=200_000)
        S = sol[:, 0]
        Rt = beta_t * S / (gamma_t * self.model.N)
    ...
    def export_json(self):
        ...
        try:
            data = {"t": self.model.t_data.tolist(), "I": self.model.I_data.tolist(), "N": int(self.pop_input.text()), "bounds": self.model.bounds.tolist(), "model": "SEIR-harmonic-v4-multi-colony"}
    ...
    def _reset_session_state(self):
        ...
        self.run_history.clear(); self.history_table.setRowCount(0); self.tabs.setTabEnabled(1, False)
        self._clear_dashboard_plots(); self.best_params_overall = None; self.best_cost_overall = float('inf')
        self.comparison_curve_item.clear(); self.update_plot()

    def _clear_dashboard_plots(self):
        for plot_widget in self.dashboard_plots.values():
            self.plot_grid_layout.removeWidget(plot_widget); plot_widget.deleteLater()
        self.dashboard_plots.clear()
        self.next_plot_row = 1; self.next_plot_col = 0
    ...
    def _add_dashboard_plot(self, name, plot_widget):
        plot_widget.setMinimumSize(450, 350); plot_widget.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
    ...
    def _rearrange_dashboard_plots(self):
        for i in reversed(range(self.plot_grid_layout.count())):
            ...
            if widget is not None and widget != self.plot_widget:
                self.plot_grid_layout.removeWidget(widget)
                widget.setParent(None)

        num_dashboard_plots = len(self.dashboard_plots)
        
        if self.plot_widget.parent() is None:
             self.plot_grid_layout.addWidget(self.plot_widget, 0, 0, 1, 2)
        
        if num_dashboard_plots == 0:
            self.plot_grid_layout.addWidget(self.plot_widget, 0, 0, 1, 2)
        else:
            self.plot_grid_layout.addWidget(self.plot_widget, 0, 0, 1, 1)

        if num_dashboard_plots > 0:
            dashboard_plot_index = 0
            for name, plot_widget in self.dashboard_plots.items():
                if dashboard_plot_index == 0:
                    self.plot_grid_layout.addWidget(plot_widget, 0, 1, 1, 1)
                else:
                    row = 1 + (dashboard_plot_index - 1) // 2
                    col = (dashboard_plot_index - 1) % 2
                    self.plot_grid_layout.addWidget(plot_widget, row, col, 1, 1)
    ...
    def show_residuals_plot(self):
        ...
        plot_widget.setLabel('left', 'Residuo (I_obs - I_pred)'); plot_widget.setLabel('bottom', 'Tiempo')
        ...
        if params is not None and len(self.model.I_data) > 0:
            ...
            try:
                k = params[-1]
                self.model.set_initial_conditions(k)
                I_pred = odeint(self.model.seir_harmonic, self.model.y0, t, args=tuple(params[:-1]), mxstep=200_000)[:, 2]
            ...
            except Exception as e: self.log(f"Error al calcular residuos: {e}", "red")
    ...
    def show_convergence_plot(self):
        ...
        plot_widget.setLabel('left', f'Costo ({self.model.loss_type})'); plot_widget.setLabel('bottom', 'Iteración')
        plot_widget.plot(result.cost_history, pen='b'); self._add_dashboard_plot("convergence", plot_widget)

    def show_distribution_plot(self):
        if not self.optimizer or not self.optimizer.archives: QMessageBox.warning(self, "Sin Datos", "No hay datos de colonias."); return
        plot_widget = pg.PlotWidget(title="Distribución de Parámetros por Colonia (Última Iter.)")
        plot_widget.setLabel('left', 'Valor Normalizado'); plot_widget.setLabel('bottom', 'Parámetro')
        low, high = self.model.LOW, self.model.HIGH; x_ticks = [list(enumerate(self.model.labels))]
        plot_widget.getAxis('bottom').setTicks(x_ticks)
        for i, archive in enumerate(self.optimizer.archives):
            normalized_archive = (archive - low) / (high - low); means = np.mean(normalized_archive, axis=0)
        ...
        plot_widget.setYRange(0, 1); self._add_dashboard_plot("distribution", plot_widget)

    def show_parallel_plot(self):
        if not self.optimizer or not self.optimizer.archives: QMessageBox.warning(self, "Sin Datos", "No hay datos de archivo."); return
        ...
        ticks = [list(enumerate(self.model.labels))]; ax = plot_widget.getAxis('bottom'); ax.setTicks(ticks); ax.setPen(pg.mkPen(None))
        ax.setTextPen(pg.mkPen('#f0f0f0' if self.is_dark_resolved else 'k'))
        plot_widget.setYRange(0, 1); archive = self.optimizer.archives[0]; low = self.model.LOW; high = self.model.HIGH
        rng = high - low; rng[rng == 0] = 1; normalized_archive = (archive - low) / rng
        for i in range(1, normalized_archive.shape[0]): plot_widget.plot(y=normalized_archive[i], pen=pg.mkPen((0, 100, 255, 30), width=1))
    ...
    def show_bounds_analysis_plot(self):
        ...
        plot_widget.setLabel('left', 'Distancia Normalizada al Límite Más Cercano'); plot_widget.setLabel('bottom', 'Parámetro')
        params = result.best_params; bounds = self.model.bounds; lows = bounds[:, 0]; highs = bounds[:, 1]
        ranges = highs - lows; ranges[ranges == 0] = 1
        normalized_params = (params - lows) / ranges; distances = np.minimum(normalized_params, 1 - normalized_params)
        colors = []
        for d in distances:
            if d < 0.05: colors.append((220, 20, 60, 200)) # Rojo
            elif d < 0.15: colors.append((255, 165, 0, 200)) # Amarillo
            else: colors.append((34, 139, 34, 200)) # Verde
        bar_item = pg.BarGraphItem(x=range(len(params)), height=distances, width=0.6, brushes=colors); plot_widget.addItem(bar_item)
        ticks = [list(enumerate(self.model.labels))]; ax = plot_widget.getAxis('bottom'); ax.setTicks(ticks)
        plot_widget.setYRange(0, 0.5)
        plot_widget.addLine(y=0.05, pen=pg.mkPen('r', style=Qt.DashLine)); plot_widget.addLine(y=0.15, pen=pg.mkPen('y', style=Qt.DashLine))
        self._add_dashboard_plot("bounds", plot_widget)

    def show_mcmc_dialog(self):
        result = self._get_selected_run_result()
        if not result: return
        
        if np.isnan(result.aic):
            QMessageBox.warning(self, "Análisis no disponible", "El análisis MCMC requiere que la optimización se haya ejecutado con la función de pérdida MSE.")
            return

        self.log("Abriendo diálogo de análisis MCMC...", "purple")
        dlg = MCMCDialog(self.model, result, self)
        # self.dialog_windows.append(dlg) # Descomentar cuando el diálogo sea más complejo
        dlg.show()
...
if __name__ == '__main__':
    if sys.platform.startswith('win'):
        ...
        import ctypes
        myappid = 'acor.seir.optimizer.v5.1'
